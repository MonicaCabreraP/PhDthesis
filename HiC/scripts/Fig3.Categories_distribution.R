...
Number_dynamic_compartments_Freq_ordered <- as_tibble(dynamic_compartments_Freq)
...
category_colors <- data.frame(Category_compartments=Number_dynamic_compartments_Freq_ordered[- grep("NULL", Number_dynamic_compartments_Freq_ordered$Category_compartments),]$Category_compartments,colors=myColors)
...
Number_dynamic_compartments_Freq_ordered$Percentage_switch <- round((Number_dynamic_compartments_Freq_ordered$Freq/nrow(Compartments[Compartments$state_compartments=="dynamic",])*100),digits = 2)
...
legend_title <- "Compartment categories"
...
print(ggplot(Number_dynamic_compartments_Freq_ordered, aes(x="",y=Percentage_switch,fill=Category_compartments)) + 
  ...
  scale_fill_manual(values = Number_dynamic_compartments_Freq_ordered$colors, 
                    breaks=Number_dynamic_compartments_Freq_ordered$Category_compartments[Number_dynamic_compartments_Freq_ordered$Percentage_switch >= 1], legend_title)+
...
## Only categories of compartments with p53 ----
...
print(ggplot(Number_dynamic_compartments_p53_Freq_ordered, aes(x="",y=Percentage_switch,fill=Category_compartments)) + 
        ...
        scale_fill_manual(values = Number_dynamic_compartments_p53_Freq_ordered$colors, 
                          breaks=Number_dynamic_compartments_p53_Freq_ordered$Category_compartments[Number_dynamic_compartments_p53_Freq_ordered$Percentage_switch >= 1], legend_title)+
        labs(title = "Switching compartments distribution of all compartments",subtitle = "Divided by all possibilities (64)")+
        xlab("")+
        ylab("% of switching compartments")+
        theme_ipsum() +
